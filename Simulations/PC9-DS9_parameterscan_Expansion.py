...
def dist_compare(dat, num_cells, div, dth):
    ...
    for rep in range(len(dat)):
        ...
        t2 = np.linspace(0, 145, 146)  # length of experiment
...
divs_DS9 = np.linspace(0.01, 0.08, 71)
dips_DS9 = np.linspace(0, 0.0020, 21)
# divs_DS9 = np.linspace(0.034, 0.036, 3)
# dips_DS9 = np.linspace(0.0014, 0.0016, 3)
...
for dip in dips_DS9:
    for di in divs_DS9:
        dt = di-dip
        print(dip, di, dt)
        sim, ks, ad = dist_compare(cFP_DS9_Erl, 1, di, dt)
...
dict = {'sim DIPs': sim_dist_list, 'DIP rate': dip_rates_DS9, 
        'division rate': div_rates_DS9, 'death rate': dth_rates_DS9, 
        'KS p-value': ks_ps, 'AD p-value': ad_ps}

df = pd.DataFrame(data=dict)

df.to_pickle('PC9-DS9_param-scan_Expansion_try2.pkl')
