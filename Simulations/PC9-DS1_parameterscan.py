...
divs_DS1 = np.linspace(0.022, 0.06, 38) * np.log(2)
dips_DS1 = np.linspace(0.0007, 0.0025, 18) * np.log(2)
p_vals_DS1 = []
div_rates_DS1 = []
dth_rates_DS1 = []
dip_rates_DS1 = []
for dip in dips_DS1:
    for di in divs_DS1:
        dt = di-dip
        # print(dip, di, dt)
        p = dist_compare(cFP_DS1_Erl, 1, di, dt)
        p_vals_DS1.append(p)
...
dict = {'DIP rate': dip_rates_DS1, 'division rate': div_rates_DS1,
        'death rate': dth_rates_DS1, 'p-value': p_vals_DS1}

df = pd.DataFrame(data=dict)

df.to_pickle('PC9-DS1_param-scan_tighterRange.pkl')
