...
def dist_compare(dat, num_cells, div, dth):
    ...
    for rep in range(len(dat)):
        ...
        t2 = np.linspace(0, 156, 157)  # length of experiment
...
divs_DS6 = np.linspace(0.01, 0.08, 71)
dips_DS6 = np.linspace(-0.0005, 0.0015, 21)
# divs_DS6 = np.linspace(0.034, 0.036, 3)
# dips_DS6 = np.linspace(0.0014, 0.0016, 3)
...
for dip in dips_DS6:
    for di in divs_DS6:
        dt = di-dip
        print(dip, di, dt)
        sim, ks, ad = dist_compare(cFP_DS6_Erl, 1, di, dt)
...
dict = {'sim DIPs': sim_dist_list, 'DIP rate': dip_rates_DS6, 
        'division rate': div_rates_DS6, 'death rate': dth_rates_DS6, 
        'KS p-value': ks_ps, 'AD p-value': ad_ps}

df = pd.DataFrame(data=dict)

df.to_pickle('PC9-DS6_param-scan_Expansion.pkl')
