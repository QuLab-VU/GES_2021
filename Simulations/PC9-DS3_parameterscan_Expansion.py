...
divs_DS3 = np.linspace(0.01, 0.08, 71)
dips_DS3 = np.linspace(-0.0015, 0.0005, 21)
# divs_DS3 = np.linspace(0.034, 0.036, 3)
# dips_DS3 = np.linspace(0.0014, 0.0016, 3)
...
for dip in dips_DS3:
    for di in divs_DS3:
        dt = di-dip
        print(dip, di, dt)
        sim, ks, ad = dist_compare(cFP_DS3_Erl, 1, di, dt)
...
dict = {'sim DIPs': sim_dist_list, 'DIP rate': dip_rates_DS3, 
        'division rate': div_rates_DS3, 'death rate': dth_rates_DS3, 
        'KS p-value': ks_ps, 'AD p-value': ad_ps}

df = pd.DataFrame(data=dict)

df.to_pickle('PC9-DS3_param-scan_Expansion.pkl')
