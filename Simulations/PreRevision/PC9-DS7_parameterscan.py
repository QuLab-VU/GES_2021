...
# will be performed
divs_DS7 = np.linspace(0.018, 0.06, 42) * np.log(2)
dips_DS7 = np.linspace(0.0016, 0.0030, 14) * np.log(2)
...
for dip in dips_DS7:
    for di in divs_DS7:
        dt = di-dip
        # print(dip, di, dt)
        p = dist_compare(cFP_DS7_Erl, 1, di, dt)
        p_vals_DS7.append(p)
...
dict = {'DIP rate': dip_rates_DS7, 'division rate': div_rates_DS7,
        'death rate': dth_rates_DS7, 'p-value': p_vals_DS7}

# Create a pandas data frame from dictionary and save as pickled object
df = pd.DataFrame(data=dict)
df.to_pickle('PC9-DS7_param-scan_tighterRange.pkl')