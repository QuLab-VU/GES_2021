...
# will be performed
divs_DS4 = np.linspace(0.023, 0.06, 37) * np.log(2)
dips_DS4 = np.linspace(0.0029, 0.0053, 24) * np.log(2)
...
for dip in dips_DS4:
    for di in divs_DS4:
        dt = di-dip
        # print(dip, di, dt)
        p = dist_compare(cFP_DS4_Erl, 1, di, dt)
        p_vals_DS4.append(p)
...
dict = {'DIP rate': dip_rates_DS4, 'division rate': div_rates_DS4,
        'death rate': dth_rates_DS4, 'p-value': p_vals_DS4}

# Create a pandas data frame from dictionary and save as pickled object
df = pd.DataFrame(data=dict)
df.to_pickle('PC9-DS4_param-scan_tighterRange.pkl')