...
def dist_compare(div, dth, prop1, prop2):
    dat = cFP_rates[cFP_rates['Cell_Line'] == 'PC9-DS9']
    num_cells = 1
    kdiv = div
    kdth = dth
    ...
    for ind, cell in enumerate(cell_tot[:int(cell_tot_s[0])]):
        ...
        t2 = np.linspace(0, 145, 146)  # in drug
    ...
    for ind, cell in enumerate(cell_tot[int(cell_tot_s[0]):]):
        ...
        t3 = np.linspace(0, 145, 146)  # in drug
...
di1 = 0.015
# di2 = np.linspace(0.01, 0.02, 2)
di2 = np.linspace(0.046, 0.080, 35)
dip1 = 0.0012
# dip2 = np.linspace(0.0001, 0.0002, 2)
dip2 = np.linspace(0.0018, 0.0028, 11)
# props1 = np.linspace(0.98, 0.99, 2)
props1 = np.linspace(0.95, 0.99, 5)
# props2 = np.linspace(0.01, 0.05, 5)
...
for d2, d2_val in enumerate(di2):
    for p2, p2_val in enumerate(dip2):
        for pr1, pr1_val in enumerate(props1):
            ls_div.append([di1, d2_val])
            ls_dip.append([dip1, p2_val])
            ls_dth.append([di1-dip1, d2_val-p2_val])
            ls_prop.append([pr1_val, 1-pr1_val])
            sim, ks, ad = dist_compare(div = [di1, d2_val], dth = [di1-dip1, d2_val-p2_val], prop1 = pr1_val, prop2 = 1-pr1_val)
...
dict = {'DIP rate': ls_dip, 'division rate': ls_div,
        'death rate': ls_dth, 'proportions': ls_prop,
        'sim DIPs': sim_dist_list, 'KS p-value': ks_ps, 'AD p-value': ad_ps}

df = pd.DataFrame(data=dict)

df.to_pickle('PC9-DS9_param-scan_twoState-ER_DivsHigh-RightDIP.pkl')
