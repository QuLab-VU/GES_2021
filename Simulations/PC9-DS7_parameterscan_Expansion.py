...
cFP_DS7_Erl = cFP_rates[(cFP_rates['Cell_Line'] == 'PC9-DS7')]
...
divs_DS7 = np.linspace(0.01, 0.08, 71)
dips_DS7 = np.linspace(0.0010, 0.0030, 21)
# divs_DS7 = np.linspace(0.034, 0.036, 3)
# dips_DS7 = np.linspace(0.0014, 0.0016, 3)
...
for dip in dips_DS7:
    for di in divs_DS7:
        dt = di-dip
        print(dip, di, dt)
        sim, ks, ad = dist_compare(cFP_DS7_Erl, 1, di, dt)
        sim_dist_list.append(sim)
        ks_ps.append(ks)
        ad_ps.append(ad)
        dip_rates_DS7.append(dip)
        div_rates_DS7.append(di)
        dth_rates_DS7.append(dt)
        print("run done")

dict = {'sim DIPs': sim_dist_list, 'DIP rate': dip_rates_DS7, 
        'division rate': div_rates_DS7, 'death rate': dth_rates_DS7, 
        'KS p-value': ks_ps, 'AD p-value': ad_ps}

df = pd.DataFrame(data=dict)

df.to_pickle('PC9-DS7_param-scan_Expansion.pkl')
